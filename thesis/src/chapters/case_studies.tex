\chapter{Testarea aplicației și rezultate}
...
Scenariul \mobilede\ validează integrarea cea mai fragilă. Utilizatorul introduce un link localizat către pagina de detaliu, împreună cu identificatorul numeric al anunțului. Backend-ul trebuie să extragă identificatorul și să parseze pagina. În primele iterații, astfel de linkuri nu funcționau deoarece parserul se aștepta la forme diferite ale URL-ului.
...
Validarea acestui scenariu urmărește patru operații. Prima este crearea contului și autentificarea prin cookie HTTP-only. A doua este salvarea automată a unei predicții reușite. A treia este redeschiderea raportului complet din pagina dedicată istoricului. Verificarea confirmă păstrarea toleranței, a metodei de agregare, a celor patru estimări individuale și a celor patru sugestii similare folosite în raportul inițial. A patra este ștergerea unui element individual sau a întregului istoric. Această ultimă funcționalitate păstrează aplicația utilizabilă atunci când sunt comparate multe anunțuri într-o singură sesiune.
...
\section{Testarea limitării ratei}
\label{sec:case-rate-limit}

Prima observație preliminară primită de la un coleg înaintea sesiunii extinse de testare a utilizabilității a fost necesitatea unui mecanism de limitare a ratei. Observația este relevantă deoarece analiza unui link poate porni scraping și mai multe modele ML, iar autentificarea implică o funcție PBKDF2 intenționat costisitoare. Fără limitare, un script simplu ar putea consuma memoria și timpul de procesor al instanței gratuite sau ar putea încerca repetat parole.

În urma observației a fost introdusă o fereastră glisantă configurabilă. Testele unitare verifică blocarea după atingerea pragului, eliberarea limitei după expirarea ferestrei, separarea adreselor IP și separarea conturilor. Un test HTTP a configurat temporar limita de login la două cereri pe minut: primele două autentificări cu date greșite au răspuns 401, iar a treia a răspuns 429 și a inclus \texttt{Retry-After: 60}. Astfel, mecanismul nu ascunde erorile normale de autentificare, dar oprește repetarea lor excesivă.

Pentru sesiunea cu colegii, valorile implicite rămân suficient de permisive pentru utilizare normală: fiecare cont poate face cinci analize pe minut, iar limita generală este de 60 de cereri pe minut pentru un IP. Dacă mai mulți participanți folosesc aceeași rețea, predicțiile sunt în continuare separate după cont; limita generală rămâne o protecție de ultim nivel. Contoarele sunt în memorie și se resetează la restart, limitare acceptabilă pentru un demo cu o singură instanță.
...
Testarea manuală și feedback-ul colegilor au scos la iveală probleme care nu erau evidente din metricile offline. Prima problemă a fost uniformizarea predicțiilor unor modele. A doua a fost imaginea greșită pe \mobilede. A treia a fost aglomerarea istoricului și pierderea contextului atunci când era păstrat doar un rezumat. A patra a fost nevoia ca utilizatorul să aleagă metoda de combinare și toleranța. Au fost cerute și explicații mai accesibile pentru termenii statistici, precum și o separare mai clară între concluzia principală și detaliile modelelor. Observația preliminară de utilizabilitate privind limitarea ratei a adăugat și o cerință operațională care nu era vizibilă în evaluarea modelelor.

Aceste observații arată valoarea iterațiilor cu interfața reală. Pipeline-ul ML poate părea complet într-un notebook, dar aplicația finală expune fricțiuni pe care doar utilizarea efectivă le dezvăluie. În proiectul \project, fiecare observație a dus la o modificare concretă: log-preț, medie ponderată, filtrare de imagini, rapoarte complete în istoric, explicații contextuale, afișare progresivă, ajustări generale de UX și protecție împotriva cererilor excesive.
